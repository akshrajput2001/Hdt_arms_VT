...
EIGEN_MAKE_TYPEDEFS(rScalar, d, 22, 22)
...
static void floatGyroframe(Eigen::Isometry3d trunk, Eigen::Isometry3d reference, Eigen::Isometry3d new_trunk)
{
  ...
  rpy_ang = CmsMath::rot2Euler(reference.linear());

  Eigen::Matrix3d temp;
  temp = CmsMath::rotateWithZ(-rpy_ang(2));
...
static Eigen::Matrix<double, 7,6> pseudoinverse(Eigen::Matrix<double,6,7> jacobian)
{
  Eigen::Matrix<double,6,6> jjt =jacobian*jacobian.transpose();
  return jacobian.transpose()*jjt.inverse();
}
