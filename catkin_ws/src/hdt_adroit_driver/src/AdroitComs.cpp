...
void AdroitComs::send_control_cmd(uint8_t addr, ControlCmd *cmd) {
	...
	//ROS_INFO("addr: %d, cmd->position: %f, cmd->velocity: %f, cmd->effort: %f", addr, cmd->position, cmd->velocity, cmd->effort);
	// set data
...
bool AdroitComs::process_error_telem(BITTelem *telem, AdroitMsg *msg) {
	...
	telem->error_severity = msg->data[1];
	telem->error_value = msg->data[2] + (msg->data[3] << 8) + (msg->data[4] << 16) + (msg->data[5] << 24);
...
//	printf("read drive %d param %d\n", addr, index);
	...
	//tm.tv_sec += PARAM_WAIT_SHORT;
	//int ret = sem_timedwait(&drive->semaphore, &tm);
	ret = timed_wait(&drive->semaphore, PARAM_WAIT_SHORT);
...
int AdroitComs::commit_parameters(int addr, AdroitDrive *drive) {
	...
	// wait for response
	drive->msg_update[PARAMETER_TELEM] = false;
	//printf("starting timed wait\n");
	ret = timed_wait(&drive->semaphore, PARAM_WAIT_LONG);
	//printf("ending timed wait\n");
	if((ret == -1) || (drive->msg_update[PARAMETER_TELEM] == false)) {
...
/*----------------------------------------------------------------------------
  timed wait
 *----------------------------------------------------------------------------*/
int AdroitComs::timed_wait(sem_t *semaphore, int sec) {
	timespec tm;
	clock_gettime(CLOCK_REALTIME, &tm);
	tm.tv_sec += sec;
	return sem_timedwait(semaphore, &tm);
}
