...
int AdroitDriver::Init(AdroitInterface *coms_interface) {
    ...
    private_node_handle.param<std::string>("name_space", name_space, "");
	...
	if(node_handle.getParam(ROBOT_DESCRIPTION, description)) {
		...
		for(it_type iterator = active_drives.begin(); iterator != active_drives.end(); iterator++) {
		  hdt_debug_telem_topics[iterator->second]="/" + (std::string)ROS_NAME + "/" + name_space + iterator->first + "/" +  "hdt_debug_telem";
		  hdt_ls_telem_topics[iterator->second]="/" + (std::string)ROS_NAME + "/" + name_space + iterator->first + "/" +  "hdt_ls_telem";
		  hdt_ms_telem_topics[iterator->second]="/" + (std::string)ROS_NAME + "/" + name_space + iterator->first + "/" +  "hdt_ms_telem";
		  hdt_hs_telem_topics[iterator->second]="/" + (std::string)ROS_NAME + "/" + name_space + iterator->first + "/" +  "hdt_hs_telem";
		  hdt_error_telem_topics[iterator->second]="/" + (std::string)ROS_NAME + "/" + name_space + iterator->first + "/" +  "hdt_error_telem";
		  hdt_param_telem_topics[iterator->second]="/" + (std::string)ROS_NAME + "/" + name_space + iterator->first + "/" +  "hdt_param_telem";
		  hdt_status_telem_topics[iterator->second]="/" + (std::string)ROS_NAME + "/" + name_space + iterator->first + "/" +  "hdt_status_telem";
		  hdt_can_control_cmd_telem_topics[iterator->second]="/" + (std::string)ROS_NAME + "/" + name_space + iterator->first + "/" +  "hdt_can_control_cmd_telem";
		  hdt_control_cmd_telem_topics[iterator->second]="/" + (std::string)ROS_NAME + "/" + name_space + iterator->first + "/" +  "hdt_control_cmd_telem";
		...
		hdt_telem_msg.state.resize(active_drives.size());
	...
	// joint command subscriber and publisher
	joint_cmd_sub = node_handle.subscribe(JOINT_CMD_TOPIC, 3, &AdroitDriver::JointCmdCallback, this);
	//joint_cmd_pub = node_handle.advertise<sensor_msgs::JointState>(JOINT_CMD_TOPIC, 3);

	// publish initial joint cmd msg
	//joint_cmd_pub.publish(joint_telem_msg);

	// HDT joint command subscriber
	hdt_cmd_sub = node_handle.subscribe(HDT_CMD_TOPIC, 3, &AdroitDriver::HDTJointCmdCallback, this);
...
void AdroitDriver::ResetDrives(void) {
	...
	for(it_type iterator = active_drives.begin(); iterator != active_drives.end(); iterator++) {
		...
		if(adroit_drives[iterator->second]->msg_update[AdroitComs::HIGH_SPEED_TELEM]) {
			...
			ROS_WARN("Did not receive response from %s", iterator->first.c_str());
...
void AdroitDriver::TimerCallback(const ros::TimerEvent &event) {
	...
	for(it_type iterator = active_drives.begin(); iterator != active_drives.end(); iterator++) {
		...
		hdt_telem_msg.state[i] = adroit_drives[iterator->second]->status_telem.state;
...
void AdroitDriver::JointCmdCallback(const sensor_msgs::JointState& msg) {
	...
	for(int i = 0; i < msg.name.size(); i++) {
		// update drive
		try {
            int addr = active_drives.at(msg.name[i]);
			if(msg.position.size() > i) adroit_drives[addr]->control_cmd.position = msg.position[i];
			if (msg.velocity.size() > i) adroit_drives[addr]->control_cmd.velocity = msg.velocity[i];
			else adroit_drives[addr]->control_cmd.velocity = DEFAULT_DRIVE_VEL;
			if(msg.effort.size() > i) adroit_drives[addr]->control_cmd.effort = msg.effort[i];
...
bool AdroitDriver::GetStatusServiceCallback(hdt_adroit_driver::GetStatus::Request& req, hdt_adroit_driver::GetStatus::Response& res) {
  ...
  for(int i = 0; i < MAX_DRIVES; i++) {
    if(adroit_drives[i]->msg_update[AdroitComs::STATUS_TELEM] == true) {
      ...
      res.state.push_back((uint32_t)adroit_drives[i]->status_telem.state);
    }
    else {
      COMS->reset_drive(adroit_drives[i]);
    }
