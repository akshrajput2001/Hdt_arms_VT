...
int ControlGroup::Init(robot_model::RobotModelPtr model, robot_state::RobotState *state, std::string robot_prefix, std::string name, std::string joint_cmd_topic, boost::function< void() > joy_cb) {
	...
	joint_space_path_as = new actionlib::SimpleActionServer<hdt_saffir_right_control::JointSpacePathAction>(nh, action_name, boost::bind(&ControlGroup::JointSpacePathCb, this, _1), false);
	joint_space_path_as->start();
	joint_space_path_ac = new actionlib::SimpleActionClient<hdt_saffir_right_control::JointSpacePathAction>(action_name, true);
	
	// task plan action servers and clients
	action_name = robot_prefix + group_name + TASK_SPACE_PATH_ACTION;
	task_space_path_as = new actionlib::SimpleActionServer<hdt_saffir_right_control::TaskSpacePathAction>(nh, action_name, boost::bind(&ControlGroup::TaskSpacePathCb, this, _1), false);
	task_space_path_as->start();
	task_space_path_ac = new actionlib::SimpleActionClient<hdt_saffir_right_control::TaskSpacePathAction>(action_name, true);

	// trajectory plan action servers and clients
	action_name = robot_prefix + group_name + TRAJECTORY_PATH_ACTION;
	trajectory_path_as = new actionlib::SimpleActionServer<hdt_saffir_right_control::TrajectoryPathAction>(nh, action_name, boost::bind(&ControlGroup::TrajectoryPathCb, this, _1), false);
	trajectory_path_as->start();
	trajectory_path_ac = new actionlib::SimpleActionClient<hdt_saffir_right_control::TrajectoryPathAction>(action_name, true);
 ...
 *----------------------------------------------------------------------------*/
void ControlGroup::JointSpacePathCb(const hdt_saffir_right_control::JointSpacePathGoalConstPtr &goal) {
	hdt_saffir_right_control::JointSpacePathResult result;
 ...
 *----------------------------------------------------------------------------*/
void ControlGroup::TaskSpacePathCb(const hdt_saffir_right_control::TaskSpacePathGoalConstPtr &goal) {
	hdt_saffir_right_control::TaskSpacePathResult result;
 ...
 *----------------------------------------------------------------------------*/
void ControlGroup::TrajectoryPathCb(const hdt_saffir_right_control::TrajectoryPathGoalConstPtr &goal) {
	hdt_saffir_right_control::TrajectoryPathResult result;
